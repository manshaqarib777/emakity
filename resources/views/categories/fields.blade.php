...
@prepend('scripts')
    <script type="text/javascript">
        ...
        var dz_var1598988452806641953ble = $(".dropzone.web_image").dropzone({
                ...
                init: function () {
                    @if(isset($category) && $category->hasMedia('web_image'))
                    dzInit(this, var1598988452806641953ble, '{!! url($category->getFirstMediaUrl('web_image','thumb')) !!}')
                    @endif
                },
                accept: function (file, done) {
                    dzAccept(file, done, this.element, "{!!config('medialibrary.icons_folder')!!}");
                ...
                removedfile: function (file) {
                    dzRemoveFile(
                        ...
                        'web_image', '{!! isset($category) ? $category->id : 0 !!}', '{!! url("uplaods/clear") !!}', '{!! csrf_token() !!}'
                    );
                }
            });
